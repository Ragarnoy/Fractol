...
/*   Updated: 2018/01/14 01:59:03 by tlernoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	compute(t_frc mb, int y, int y_max)
{
	...
	while (++pt.x < W_HGHT && ((pt.y = y - 1)))
	{
		while (++pt.y < y_max && ((i = -1)))
		{
			...
			while (fabs((z_r * z_r) + (z_i * z_i))
					< 1 && ++i < mb.i_max)
				calculus(&z_r, &z_i);
			if (z_i < -sqrt(3) / 4 + 0.000001)
				putpixel(mb.cl[0], pt);
			else if (z_i > sqrt(3) / 4 - 0.000001)
...
void		newton3(t_env *env, t_pnt *thrd)
{
	if (!env->f[7].init)
	{
		...
		env->f[7].y[0] = -1;
		env->f[7].y[1] = 1;
		env->f[7].zm_x = W_WDTH / (env->f[7].x[1] - env->f[7].x[0]);
		env->f[7].zm_y = W_HGHT / (env->f[7].y[1] - env->f[7].y[0]);
		env->f[7].i_max = 25;
		env->f[7].cl[0].h = 30;
		env->f[7].cl[0].s = 1;
		env->f[7].cl[0].l = 1;
