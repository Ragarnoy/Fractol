/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   f_newton.c                                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tlernoul <tlernoul@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/01/13 19:59:11 by tlernoul          #+#    #+#             */
/*   Updated: 2018/01/14 02:49:18 by tlernoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/fractol.h"

static void	calculus(double *z_r, double *z_i)
{
	double tmp;
	double cl;

	cl = 3 * ((((*z_r * *z_r) - (*z_i * *z_i)) * ((*z_r * *z_r) -
					(*z_i * *z_i))) + ((2 * *z_r * *z_i) * (2 * *z_r * *z_i)));
	tmp = *z_r;
	*z_r = ((*z_r / 3) * 2) + ((*z_r * *z_r) - (*z_i * *z_i)) / cl;
	*z_i = ((*z_i / 3) * 2) - (2 * tmp * *z_i) / cl;
}

static void	compute(t_frc mb, int y, int y_max)
{
	t_pnt			pt;
	unsigned int	i;
	double			z_r;
	double			z_i;

	pt.x = -1;
	while (++pt.x < W_HGHT && ((pt.y = y - 1)))
	{
		while (++pt.y < y_max && ((i = -1)))
		{
			z_r = (double)pt.x / mb.zm_x + mb.x[0];
			z_i = (double)pt.y / mb.zm_y + mb.y[0];
			while (fabs((z_r * z_r) + (z_i * z_i))
					< 1.0000000001 && ++i < mb.i_max)
				calculus(&z_r, &z_i);
			if (i == mb.i_max)
				putpixel(mb.cl[0], pt);
			else
				picker(mb.cl[1], (float)i / (float)mb.i_max, pt);
		}
	}
}

void		newton2(t_env *env, t_pnt *thrd)
{
	if (!env->f[6].init)
	{
		env->f[6].x[0] = -1;
		env->f[6].x[1] = 1;
		env->f[6].y[0] = -1;
		env->f[6].y[1] = 1;
		env->f[6].zm_x = W_WDTH / (env->f[6].x[1] - env->f[6].x[0]);
		env->f[6].zm_y = W_HGHT / (env->f[6].y[1] - env->f[6].y[0]);
		env->f[6].i_max = 15;
		env->f[6].cl[0].h = 30;
		env->f[6].cl[0].s = 1;
		env->f[6].cl[0].l = 0;
		env->curp = 6;
		env->f[6].zml = 0;
		env->f[6].init = 1;
		return ;
	}
	compute(env->f[6], thrd->x, thrd->y);
}
